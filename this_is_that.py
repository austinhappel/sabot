...
class ThisIsThat(BotPlugin):
    ...
    def parse_question(self, mess, must_respond=False):
        ...
        if key in self.keys():
            print key + ' is ' + self.__getitem__(key)
        elif must_respond is True:
            print "I don't know what " + key + " is."
    ...
    def callback_message(self, conn, mess):
        ...
        if re.search(self.short_name_regex, message) != None:
            # is this a command?
            if re.search(self.question_regex, message) == None and \
            re.search(r'\bis\b', message) != None:
                self.parse_command(mess)

            # is this a request?
            if re.search(self.question_regex, message) != None:
                self.parse_question(mess, True)

        else:
            # is this a request?
            if re.search(self.question_regex, message) != None:
                self.parse_question(mess)
